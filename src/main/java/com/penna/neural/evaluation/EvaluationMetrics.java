...
public class EvaluationMetrics {
    ...
    /**
     * Computes accuracy of predictions with respect to a gold standard true
     * labels. Accuracy is computed as the fraction of predictions whose label
     * with maximum value matches the label with maximum value in the gold set.
     * 
     * @param predictions the prediction labels
     * @param golds the corresponding gold standard true labels
     * @return the accuracy of the predictions
     * @throws NoLabelException if prediction list and gold list do not match in
     *             length
     */
    public static double accuracy(List<DoubleMatrix> predictions, List<DoubleMatrix> golds)
    ...
    /**
     * Computes average cosine between a prediction and a gold label, across all
     * examples in input.
     * 
     * @param predictions the prediction labels
     * @param golds the corresponding gold standard true labels
     ...
     *             length
     */
    public static double averageCosine(List<DoubleMatrix> predictions, List<DoubleMatrix> golds)
            throws NoLabelException {
        if (predictions.size() < 1 || predictions.size() != golds.size()) {
            throw new NoLabelException("Predictions and gold standard labels are not valid");
        }
        ...
        for (int i = 0; i < predictions.size(); i++) {
            ...
            if (predictedLabel.columns != 1 || predictedLabel.rows != trueLabel.rows
                    || predictedLabel.columns != trueLabel.columns) {
                throw new NoLabelException("Predictions and gold standard labels are not valid");
            }
            if (predictedLabel.norm2() != 0 || trueLabel.norm2() != 0) {
                double cosine = predictedLabel.mul(trueLabel).sum();
                totPredictions++;
                avgCosine += cosine;
            }
        }
        return avgCosine / totPredictions;
    }
}